...
.ratio_post <-
  function(user,
    ...
    if(sum(c(reply, quote, retweet, favorite), na.rm = TRUE) < 1) {
      if(verbose) {
        msg <-
          sprintf(
            paste0(
              "`reply`, `quote`, `retweet`, and `favorite` are each `FALSE`. ",
    ...
    if(reply & quote) {
      if(verbose) {
        msg <-
          sprintf(
            paste0(
              "Both `reply`ing and `quote`ing to the same tweet is probably overkill ",
...
.ratio_post_possibly <-
  purrr::partial(.ratio_post, otherwise = config$post_status_id_sentinel)
